...
Реализовано в пакете `reels/` (см. README). Упрощения относительно целевой архитектуры
и что сделано иначе:

| Целевая архитектура | MVP |
|---|---|
| Отдельные сервисы `bot` / `orchestrator` / `analysis` / `render` | Один Python-пакет с теми же модулями; границы модулей совпадают с будущими сервисами |
| Очередь Redis + Celery, GPU/CPU-воркеры | Задачи в процессе бота: `asyncio` + потоки, семафор на число одновременных задач |
| S3 / MinIO | Локальный диск (`REELS_DATA_DIR`) |
| PostgreSQL + pgvector | SQLAlchemy async: SQLite по умолчанию, PostgreSQL через `REELS_DATABASE_URL`; эмбеддингов нет — подбор кадров по текстовым описаниям (LLM) или по оценке качества (эвристика) |
| Правки через JSON Patch к EDL | LLM возвращает полный обновлённый `PlanDraft` (компактный план сцен), builder заново строит EDL; все версии хранятся (`/versions`, `/undo`); неизменённые клипы берутся из кеша рендера |
| Webhook | Long polling (проще развёртывание) |
| Разбор референса: OCR, классы переходов, LUT | Ритм (длительности планов), BPM, доля склеек в бит, речь, описание планов и структуры мультимодальной моделью |
| Визуальный QA моделью | Технический QA: длительность, размер, чёрные и застывшие кадры, громкость |
| Музыкальная библиотека с тегами | Трек пользователя или первый трек из `assets/music/` |
| Автокадрирование с трекингом | Статическая точка интереса (лицо по Haar-каскаду или центр «энергии» кадра) + зум/Ken Burns |

Ключевой принцип сохранён: **LLM планирует (`PlanDraft`), код исполняет (`EDL` → FFmpeg)**. Без
ключа Anthropic весь цикл работает на эвристиках.

Следующие шаги (v1): Redis-очередь и отдельные воркеры, S3, эмбеддинги кадров (SIGLIP) для
подбора, OCR и классификация переходов в референсе, визуальный QA, трекинг-рефрейм, TTS-озвучка,
загрузка референса по ссылке.
